...
public class GerenteDeImagens {
    ...
    public boolean setImagemNaJlabel(String path, JLabel jLabel) {
        ...
        if ((new File(path)).isFile()) {
            try {
                ...
                Logger.getLogger(GerenteDeImagens.class.getName()).log(Level.SEVERE, (String)null, var5);
            }
    ...
    public void gravarImagem(String path, int width, int height, String novoCaminho) {
        if ((new File(path)).isFile()) {
            try {
                ...
                BufferedImage new_img = new BufferedImage(width, height, 1);
                Graphics2D g = new_img.createGraphics();
                g.drawImage(image, 0, 0, width - 2, height - 2, (ImageObserver)null);
                ImageIO.write(new_img, "JPG", new File(novoCaminho));
            } catch (IOException var8) {
                JOptionPane.showMessageDialog((Component)null, "Ocorreu um erro au salvar imagem!\nDescrição do erro: " + var8.getMessage());
            }
        }
    ...
    public String escolherImagem(JLabel jLabel) {
        ...
        if (chooser.showOpenDialog((Component)null) == 0) {
            ...
            Image image = icone.getImage().getScaledInstance(jLabel.getWidth() - 2, jLabel.getHeight() - 2, 1);
            jLabel.setIcon(new ImageIcon(image));
            GerenciadorIO.getPropriedades("ultimoPath", chooser.getCurrentDirectory().getPath());
            return this.urlImagem = chooser.getSelectedFile().getAbsolutePath();
        } else {
            return "";
        }
    }
